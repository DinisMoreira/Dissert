...
class MCTS(AbstractPlanner):
    ...
    def evaluate(self, state, observation, total_reward=0, depth=0):
        ...
        for h in range(depth, self.config["horizon"]):
            ...
            action_index = self.np_random.choice(len(actions), 1, p=np.array(probabilities))[0]

            action = actions[action_index]
            #self.np_random.choice(actions, 1, p=np.array(probabilities))[0]
...
class MCTSNode(Node):
    ...
    def expand(self, actions_distribution):
        ...
        if isinstance(actions[0], int):
            ###print("INT - No problem")
            for i in range(len(actions)):
                if actions[i] not in self.children:
                    self.children[actions[i]] = type(self)(self, self.planner, probabilities[i])
        
        elif isinstance(actions[0], list):
            ###print("LIST")
            for i in range(len(actions)):
                if tuple(actions[i]) not in self.children:
                    self.children[tuple(actions[i])] = type(self)(self, self.planner, probabilities[i])

        else:
            raise ValueError("Action type not suported for expansions")
